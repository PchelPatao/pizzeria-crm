    ...
    <header class="bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800 sticky top-0 z-10 shadow-sm">
      <div class="max-w-7xl mx-auto px-6 py-3 flex justify-between items-center">
        <div class="flex items-center gap-3">
          <div class="w-8 h-8 rounded-lg bg-gradient-to-br from-orange-500 to-red-600 flex items-center justify-center text-white text-sm font-bold shadow">🍕</div>
          <div>
            <h1 class="text-base font-bold text-slate-900 dark:text-white leading-tight">Pizzeria CRM</h1>
            <p class="text-xs text-slate-400 leading-tight">Управление заказами</p>
          </div>
        </div>
        <div class="flex items-center gap-3">
          <button
          ...
          <button
            ...
            class="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-lg font-medium text-sm transition-colors shadow-sm flex items-center gap-1.5"
          >
          ...
          <router-link
            to="/register"
            class="bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-300 px-4 py-2 rounded-lg font-medium text-sm transition-colors shadow-sm flex items-center gap-1.5"
    ...
    <div class="max-w-7xl mx-auto px-6 py-6">
      ...
      <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
        <div class="bg-white dark:bg-slate-900 rounded-xl p-4 border border-slate-200 dark:border-slate-800 shadow-sm">
          <p class="text-xs text-slate-500 font-medium mb-1">Всего заказов</p>
          <p class="text-2xl font-bold text-slate-900 dark:text-white">{{ store.orders.length }}</p>
        </div>
        <div class="bg-white dark:bg-slate-900 rounded-xl p-4 border border-slate-200 dark:border-slate-800 shadow-sm">
          <p class="text-xs text-slate-500 font-medium mb-1">Заказов сегодня</p>
          <p class="text-2xl font-bold text-slate-900 dark:text-white">{{ todayOrders }}</p>
        </div>
        <div class="bg-white dark:bg-slate-900 rounded-xl p-4 border border-slate-200 dark:border-slate-800 shadow-sm">
          <p class="text-xs text-slate-500 font-medium mb-1">В работе</p>
          <p class="text-2xl font-bold text-amber-600">{{ store.orders.filter(o => o.status === 'Preparing').length }}</p>
        </div>
        <div class="bg-white dark:bg-slate-900 rounded-xl p-4 border border-slate-200 dark:border-slate-800 shadow-sm">
          <p class="text-xs text-slate-500 font-medium mb-1">Выручка</p>
          <p class="text-2xl font-bold text-green-600">{{ totalRevenue.toLocaleString('ru-RU') }} ₽</p>
        </div>
      </div>
      ...
      <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
        <div
          ...
          :key="status"
          class="bg-white dark:bg-slate-900 rounded-xl border-t-4 border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden"
        ...
        >
          ...
          <!-- Cards -->
          <div class="p-3 space-y-2.5 min-h-[480px]">
